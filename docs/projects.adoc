...
* Owner - Same as Editor, but can also manage the project settings itself, including assigning other users to roles

In addition, a *Viewer* role is auto-created for projects in <<#access-mode, Private or Custom>> access mode, and grants read access to the project's content. It is not a manually-assigned project role.

=== Capability matrix

Project roles are CMS-level conveniences — underneath, they grant combinations of the seven <<content/permissions#permission-types, content permissions>>. The mapping is:

[cols="1,^1,^1,^1,^1,^1,^1,^1", options="header"]
|===
| Role        | READ | CREATE | MODIFY | DELETE | PUBLISH | READ_PERMISSIONS | WRITE_PERMISSIONS
| Viewer      | ✓    |        |        |        |         |                  |
| Contributor | ✓    |        |        |        |         |                  |
| Author      | ✓    | ✓      | ✓      | ✓      |         |                  |
| Editor      | ✓    | ✓      | ✓      | ✓      | ✓       | ✓                | ✓
...
NOTE: Cells marked `?` are pending verification.
...
By assigning and configuring Enonic apps for a projects, their Schemas, Extensions and other functionality will be available within the scope of the project. For example, you might have a project for a blog, where you assign a blogging application that provides content types and components for creating blog posts. Configuration of these applications takes effect immediately, without redeploying the app.

NOTE: The configuration *values* themselves are not exposed through the GraphQL API by default. Because they may hold secrets (API keys, tokens), they are kept server-side unless an app deliberately exposes selected fields. The same applies to <<pages/site#_site_configuration, site configuration>>.

<<content/layering#, Layered projects>> automatically inherit application settings from parent layers, unless they are overridden in the child layer. This means that if you assign an application to a parent layer, it will be available in all child layers, unless you explicitly override it in a child layer.

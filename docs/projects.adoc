...
NOTE: The configuration *values* themselves are not exposed through the GraphQL API by default. Because they may hold secrets (API keys, tokens), they are kept server-side unless an app deliberately exposes selected fields. The same applies to <<pages/site#site_configuration, site configuration>>.

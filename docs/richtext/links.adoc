...
In Content Studio, the HtmlArea editor provides a link dialog that allows editors to insert links to content, media and external URLs. The editor also supports editing existing links, including changing the link type and target.

.The link dialog is available from the editor toolbar
image::link-dialog.png[Link dialog with all available options, 830w]
...
A front-end consumes `processedHtml` together with the typed `links` data, correlating each `data-link-ref` tag with its `ref` to route links its own way (for example mapping a referenced content item to an application route). Enonic's front-end toolkits handle this mapping for you — see <<../frontend#, Front-end integration>> and the https://developer.enonic.com/learn/react-spa/stable/rich-text[React SPA tutorial] for worked examples.

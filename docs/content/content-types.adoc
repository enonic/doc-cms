...
Every content item has a built-in display name field, stored in the top-level `displayName` property. It is the item's human-readable title. The display name is shown in Content Studio lists and content selectors, and returned as `displayName` by the APIs for every content type.

.Screenshot of the display name field in Content Studio
image::displayname.png["Display name field as seen in the Content Studio at the top of the content editor form - in the example the content type is a folder", 546px]
...
.Don't — the title duplicated as a form item
[source,yaml]
----
kind: "ContentType"
title: "Article"
...
.Do — the built-in display name, relabelled for this type
[source,yaml]
----
kind: "ContentType"
title: "Article"
...
Two optional settings adapt the display name to a content type, and neither one adds a form item:

`displayNamePlaceholder`:: Sets a different label for the display name field in the content form — editors see `<Article heading>` instead of the generic `<Display Name>`. Supports <<../schemas/i18n#, localization>> via the `text`/`i18n` object pattern.
`displayNameExpression`:: Composes the value from other form fields, for items whose name is derived rather than typed — see <<#display-name-expressions, Display name expressions>>.

NOTE: Composing the display name is the client's job, not the platform's — XP stores whatever value it is given. Content Studio is responsible for calculating the `displayNameExpression`, and what derives the content item's URL name (`_name`) from the display name as the editor types. Content created through the APIs or by import must supply its own `displayName`.
...
<3> *displayNamePlaceholder* (optional) Sets a different label for the built-in <<#display-name, display name>> field — editors here see `<Full Name>` instead of the generic `<Display Name>`. The person's name is the display name, so it needs no form item of its own.
...
<9> *allowChildContentType* (optional) If specified, only content of content types matching specified criteria will be allowed to be created or moved under content of this content type. The pattern matching used is the same as the one described in <<../schemas/form-items/contentselector#allowContentType, MATCH>>.
<10> *form* (optional) The <<../schemas/forms#,Form>> definition for your content type — a list of form items.
<11> Input-specific settings, like `allowContentType` here, are placed directly on the input — there is no `config:` wrapper in YAML. See each <<../schemas/form-items#, form item>> for its available settings.

NOTE: `allowChildContentType` has no effect if `allowChildContent` is set to `false`


[[display-name-expressions]]
== Display name expressions

Every content item has a standard editorially managed display name field, stored in the `displayName` property. Two optional top-level settings instruct Content Studio (or any other client) to generate display names instead of relying on the basic `displayName` field. The two settings work differently: `displayNameExpression` composes and persists the actual `displayName` value while the content is being edited, whereas `displayNameListExpression` only changes how items are presented in Content Studio's list views.
...
See the https://developer.enonic.com/docs/guillotine[Guillotine documentation] for the full list of queries, arguments, and DSL options.

...
NOTE: Guillotine queries the `master` branch by default, so the query above sets `branch: "draft"`. The query resolves `NEW`, `MODIFIED`, `MOVED`, and `UNPUBLISHED` change states. See the https://developer.enonic.com/docs/guillotine/stable[Guillotine documentation] for branch selection details.

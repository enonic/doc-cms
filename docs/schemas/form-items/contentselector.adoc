...
.Standard list mode with content items listed in a dropdown below the input field.
image::contentselector.png[An input box with a dropdown listing content items below it, 546w]

.Tree mode with content items in a tree structure.
image::contentselector-tree.png[Drill in tree structure, 546w]

.Two items have been selected.
image::contentselector-selected.png[Two items selected, 546w]
...
<2> *allowPath* is used to limit the path of the content that may be selected using the <<allowPath, allowPath patterns>>. By default, Content Selector
only displays content from the same site.
<3> *treeMode* toggles between tree and flat modes. Default value is `false` (flat mode). +
image:contentselector-tree.png[Input box with a dropdown with a tree structure, 350]
...
`LEGACY`:: simulates the pattern-matching behaviour that was the default before XP 7.7.0.
`MATCH`:: the current default mode, with notable changes from `LEGACY`.
...
In the GraphQL API the reference is resolved to the referenced content, so you can query its fields in the same request rather than following the ID yourself. See <<../../content/references#,References>> for query examples.

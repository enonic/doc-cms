...
TIP: HtmlArea values contain Enonic-specific markup (`content://` links, image references, macros) that must be resolved before rendering. The headless API exposes a processed variant — see <<../../richtext#html-processing, HTML processing>>.
...
The value is stored as HTML. The HTML contains Enonic-specific markup for internal links, images, and macros that need processing before rendering.

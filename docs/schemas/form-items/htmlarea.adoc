...
HtmlArea persists raw markup that is strictly validated against the editor instance's allowed options (`include`, `exclude` and `allowHeadings`). The markup uses Enonic-specific conventions for internal links, images and macros — see <<../../richtext#raw-format, Rich text>> for the format details and <<../../richtext#html-processing, processing>> for how they are resolved.

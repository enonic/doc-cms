...
image::imageselector.png[Input box with a grid of images to select below it, 546w]
...
The stored value is the content ID of the selected image. Like <<contentselector#, ContentSelector>>, the value is a reference to the image content item.
...
In the GraphQL API the reference is resolved to the referenced image content, so you can query its fields (and image URLs) in the same request. See <<../../content/references#,References>> for query examples.

...
NOTE: A FieldSet accepts only `label` and `items`. Unlike inputs, <<item-set#, ItemSets>> and <<option-set#, OptionSets>>, it supports neither `helpText` nor `occurrences` — the label alone describes the group.

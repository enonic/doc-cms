...
The stored value is the name (string) of an <<../../content/attachments#, attachment>> on the content item. The file itself is stored as an attachment, not as a separate content. See <<../../content/attachments#, Attachments>> for details on how attachments are stored and indexed.

.Single attachment
[source,JSON]
----
{
  "myattachment": "document.pdf"
}
----

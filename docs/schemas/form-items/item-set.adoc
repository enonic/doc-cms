...
- type: "ItemSet"
  ...
  helpText: "Add one entry per phone number"  # <3>
  occurrences:  # <4>
    min: 0
    max: 0
  items:
...
<3> *helpText* (optional) — explanatory text shown below the set's label. Like `label`, supports <<../i18n#, localization>> via the `text`/`i18n` object pattern.
<4> *occurrences* control the minimum and maximum instances of the ItemSet that may be created. `max: 0` means unlimited.

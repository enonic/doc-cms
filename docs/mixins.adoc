...
The file with X-data schema must be placed in the folder `site/x-data/[name]` and called `[name].yaml`. For example, _site/x-data/address/address.yaml_.
...
./src/main/resource/site/site.yaml
[source,yaml]
----
kind: "Site"
xdata:

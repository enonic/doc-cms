...
Beyond `raw` and `processedHtml`, the HtmlArea GraphQL type also exposes `images`, `links`, and `macros` as typed fields — useful when a front-end needs structured data about references rather than parsing HTML. See the https://developer.enonic.com/docs/guillotine[Guillotine documentation] for the full type definition.
